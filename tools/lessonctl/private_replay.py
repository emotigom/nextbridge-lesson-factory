...
def verify_private_package(course,zip_path:Path):
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        with zipfile.ZipFile(zip_path) as z:
            ...
            z.extractall(td)
        dirs=[p for p in td.iterdir() if p.is_dir()]; base=dirs[0] if len(dirs)==1 else td
        manifests=sorted(base.glob('release_manifest*.json'))
        if not manifests: raise QAError('release manifest missing')
        manifest=load_jsonish(manifests[0]); assets=manifest.get('assets',[])
        for a in assets:
            f=base/a['file']
            if not f.exists() or f.stat().st_size!=a['size'] or sha256(f)!=a['sha256']: raise QAError('asset mismatch: '+a['file'])
        out['checks'].append({'name':'manifest_assets','status':'PASS','count':len(assets)})
        ...
        html_asset=next((a for a in assets if a.get('role')=='student-runtime'),None); ppt_asset=next((a for a in assets if a.get('role')=='prototype-presentation'),None); ppt_qa_asset=next((a for a in assets if a.get('role')=='prototype-qa-report'),None); render_asset=next((a for a in assets if a.get('role')=='prototype-render-evidence'),None); preview_asset=next((a for a in assets if a.get('role')=='prototype-preview'),None); runner_asset=next((a for a in assets if a.get('role')=='runtime-test-runner'),None); report_asset=next((a for a in assets if a.get('role')=='runtime-qa-report'),None)
        if html_asset:
        ...
        if ppt_asset and ppt_qa_asset:
            ...
            if qj.get('sha256')!=ppt_asset['sha256'] or qj.get('slideCount')!=5 or qj.get('passed') is not True or qj.get('failures'): raise QAError('prototype QA report cross-contract failed')
        ...
        if ppt_asset and render_asset:
            rj=load_jsonish(base/render_asset['file']); ge=load_jsonish(ROOT/course['goldenFixture'])['expected']['prototype']; ar=rj.get('artifactRender',{}); st=rj.get('slidesTest',{}); pages=ar.get('pages',[])
