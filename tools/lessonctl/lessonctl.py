...
def main():
    ...
    ev=sub.add_parser('evidence'); evsub=ev.add_subparsers(dest='evcmd',required=True); evv=evsub.add_parser('verify'); evv.add_argument('--course',required=True); evv.add_argument('--file',required=True); evs=evsub.add_parser('subject'); evs.add_argument('--course',required=True); evs.add_argument('--gate',required=True); evp=evsub.add_parser('plan'); evp.add_argument('--course',required=True); evsc=evsub.add_parser('scaffold'); evsc.add_argument('--course',required=True); evsc.add_argument('--out-dir',required=True)
    ...
    try:
        ...
        if a.cmd=='evidence':
            ...
            if a.evcmd=='scaffold':
                rep=scaffold_evidence(a.course,ROOT/Path(a.out_dir)); print(json.dumps(rep,ensure_ascii=False,indent=2)); return
