...
4. 학생 화면 문장과 교사용 대본을 섞지 마세요.
5. 각 차시 storyboard는 실제 승인된 상태인 경우에만 `APPROVED`로 넣으세요.
6. 승인되지 않은 차시는 storyboard 배열에 넣지 말고 approval을 `PENDING`으로 두세요.
7. `designState`는 임의로 높이지 말고 현재 승인 상태와 정확히 맞추세요.
8. `ALL_CONTENT_APPROVED`일 때만 `qualityScore`를 포함하세요.
9. ChatGPT 대화 전문이나 개인 정보는 handoff에 넣지 마세요.
10. `privateConversationEvidence.detailPublished`는 반드시 false로 두세요.
11. 출력은 설명문 없이 유효한 JSON 하나만 제공하세요.
12. 스키마는 Nextbridge Factory의 `contracts/chatgpt-handoff.schema.json` 1.0.0을 따르세요.
```
